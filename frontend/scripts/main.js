...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 3. STATE MANAGEMENT ---
    let allProperties = []; 
    let universityProperties = []; // Store university-specific results
    let activeFilters = {
        searchTerm: '', minPrice: null, maxPrice: null, bedrooms: 'any', 
        bathrooms: 'any', availableDate: 'any', isFurnished: false,
        university: 'all' // New: university filter
    };

    // --- 4. CORE FUNCTIONS ---

    async function fetchUniversityData(universityName) {
        const query = `
            query GetUniversityCommute($university: UniversityNameEnum!) {
                get_university_commute_profile(university_name: $university, limit: 500) {
                    directWalkOptions {
                        items {
                            property {
                                listing_id
        ...
        try {
            const response = await fetch(API_URL, {
                ...
                body: JSON.stringify({ 
                    query,
                    variables: { university: universityName }
                }),
            });
            ...
            return result.data.get_university_commute_profile.directWalkOptions.items.map(item => ({
                ...item.property,
                walkTimeToUniversityMinutes: item.walkTimeToUniversityMinutes,
                distanceToUniversityKm: item.distanceToUniversityKm
            }));

        } catch (error) {
            console.error('Failed to fetch university data from GraphQL API:', error);
            return null;
        }
    }
    ...
    async function fetchData() {
        ...
        try {
            ...
            console.error('Failed to fetch data from GraphQL API:', error);
    ...
    async function applyFiltersAndRender() {
        ...
        if (activeFilters.university !== 'all') {
            if (!universityProperties || universityProperties.length === 0) {
                listingsContainer.innerHTML = '<p class="text-center text-textSecondary py-10">正在加载大学附近房源...</p>';
                const universityData = await fetchUniversityData(activeFilters.university);
                if (universityData) {
                    universityProperties = universityData;
                } else {
                    renderError('无法加载大学附近的房源。请稍后再试。');
                    return;
                }
            }
            let propertiesData = universityProperties;
        } else {
            ...
            universityProperties = []; // Clear university data when switching back to all
        }
