...
> **最后更新**: 2025-09-05 (Memory Bank完成全面精简)
> **重要更新**: 虚拟滚动成功启用，渲染性能提升83%; Memory Bank文件精简完成; PC 详情页样式统一（页灰卡白、1200px 容器、左右 32px 内边距、全局 tokens 对齐）；图片计数器复刻与 ESLint 清理；通勤策略文档对齐（后端 Google Directions + Haversine 回退）；PC/移动端“筛选入口统一”——仅保留“筛选”按钮（去除快捷项/下拉，统一打开 FilterPanel）
...
│   ├── FilterTabs.vue        # 筛选触发器（仅打开 FilterPanel；无预设/下拉）
