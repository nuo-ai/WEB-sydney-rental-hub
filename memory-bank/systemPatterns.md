...
### 5. 可视化令牌与跨端桥接模式
- **模式**: 建立一个独立于业务应用的可视化工具站，用于实时调参、预览所有设计 Token，并形成“导出→回填”的跨端同步闭环。
- **实现**:
  - **工具站**: `tools/design-site-astro` 作为“单一事实来源”，提供 `/tokens`（实时调参）与 `/components`（多状态画廊）页面。
  - **数据源**: `public/tokens/srh.json` 存放最原始的 Token 定义（颜色/间距/字体/字重/字距/级别等）。
  - **跨端桥接**:
    - Astro 站点：通过 JS 读取 JSON 并动态应用为 CSS 变量（`--srh-*`）。
    - uni-app：在 `apps/uni-app/src/uni.scss` 中定义 `$srh-*` SCSS 变量，并桥接为 `:root --srh-*`，确保 H5 端可运行时对齐，小程序端静态构建生效。
- **工作流**:
  1) 在 Astro 工具站调参至满意效果。
  2) 导出 SCSS 片段。
  3) 回填到 `uni.scss` 的 `$srh-*` 变量定义。
  4) 重新构建 uni-app，实现多端视觉统一。
- **价值**: 替代 Storybook，提供更轻量、更聚焦于“调参-预览-导出”的 Token 管理方案，满足“所有 Token 直观渲染”的核心诉求。
