...
type UniversityName = keyof typeof UNIVERSITY_MAPPING;
...
interface CommuteProperty {
  property: Property;
  walkTimeToUniversityMinutes?: number;
...
class SydneyRentalMCP {
  ...
  private async handleStructuredSearch(args: any) {
    try {
      const params = this.validateSearchParams(args);
      
      let results: CommuteProperty[] = [];
      let searchStats = {
        totalFound: 0,
        avgRent: 0,
        minDistance: 0,
  ...
  private async handlePropertyDetail(args: any) {
    try {
      if (!args.listing_id) {
  ...
  private async handleUniversityComparison(args: any) {
    try {
      ...
      for (const university of args.universities) {
        const searchParams: SearchParams = {
          university: university as UniversityName,
  ...
  private validateSearchParams(args: any): SearchParams {
    ...
    if (args.university) {
      ...
      params.university = args.university as UniversityName;
    }
  ...
  private async searchByUniversity(params: SearchParams) {
    const query = `
    ...
    let properties: CommuteProperty[] = walkOptions.items || [];
  ...
  private async searchGeneral(params: SearchParams) {
    const query = `
    ...
    let properties: CommuteProperty[] = allProperties.map((prop: Property) => ({
      property: prop
    }));
  ...
  private applyFilters(properties: CommuteProperty[], params: SearchParams): CommuteProperty[] {
  ...
  private calculateStats(properties: CommuteProperty[], searchType: string) {
  ...
  private formatSearchResults(properties: CommuteProperty[], stats: any, params: SearchParams): string {
    let result = '🎯 搜索条件:\n';
  ...
  private formatPropertyDetail(property: any): string {
    let result = `🏠 房源详细信息\n\n`;
    ...
    // 房屋设施
    const features = [];
    if (property.has_air_conditioning) features.push('空调');
    if (property.is_furnished) features.push('带家具');
    if (property.has_balcony) features.push('阳台');
    if (property.has_dishwasher) features.push('洗碗机');
    if (property.has_laundry) features.push('洗衣设施');
    if (property.has_built_in_wardrobe) features.push('内置衣柜');
    if (property.has_gym) features.push('健身房');
    if (property.has_pool) features.push('游泳池');
    if (property.has_parking) features.push('停车位');
    if (property.allows_pets) features.push('允许宠物');
  ...
  private formatUniversityComparison(results: any[], params: any): string {
    let result = '📊 大学附近房源对比\n\n';
    ...
    results.forEach(item => {
      if (!item.error && item.sampleProperties.length > 0) {
        ...
        item.sampleProperties.slice(0, 2).forEach((prop: any, index: number) => {
